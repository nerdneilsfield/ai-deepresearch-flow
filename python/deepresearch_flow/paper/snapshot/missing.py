...
def _show_basic_missing(conn: sqlite3.Connection) -> None:
    ...
    table = Table(
        title="Basic Content Missing", header_style="bold cyan", title_style="bold magenta"
    )
...
def _show_template_coverage(conn: sqlite3.Connection) -> None:
    ...
    table = Table(title="Template Coverage", header_style="bold cyan", title_style="bold magenta")
    ...
    for template in templates:
        cursor = conn.execute(
            "SELECT COUNT(*) as cnt FROM paper_summary WHERE template_tag = ?", (template,)
        )
        has_count = cursor.fetchone()["cnt"]
        missing = total - has_count
        coverage = has_count / total * 100 if total else 0.0

        table.add_row(template, str(has_count), str(missing), str(total), f"{coverage:.1f}%")
...
def _show_translation_coverage(conn: sqlite3.Connection) -> None:
    ...
    table = Table(
        title="Translation Coverage", header_style="bold cyan", title_style="bold magenta"
    )
    ...
    for lang in languages:
        cursor = conn.execute(
            "SELECT COUNT(DISTINCT paper_id) as cnt FROM paper_translation WHERE lang = ?", (lang,)
        )
        has_count = cursor.fetchone()["cnt"]
        missing = total - has_count
        coverage = has_count / total * 100 if total else 0.0

        table.add_row(lang, str(has_count), str(missing), str(total), f"{coverage:.1f}%")
...
def _show_static_missing(conn: sqlite3.Connection, static_dir: Path) -> None:
    ...
    table = Table(
        title="Static File Existence", header_style="bold cyan", title_style="bold magenta"
    )
...
def export_missing(opts: ExportMissingOptions) -> None:
    ...
    try:
        ...
        if opts.missing_type == "source_md":
            ...
            if not opts.template:
                raise click.ClickException(
                    "--template is required when exporting missing templates"
                )
        ...
        # Print summary
        Console().print(
            f"[bold cyan]Exporting {len(missing_papers)} papers missing {title}[/bold cyan]"
        )
        ...
        if opts.output_json:
            ...
            opts.output_json.write_text(
                json.dumps(missing_papers, ensure_ascii=False, indent=2), encoding="utf-8"
            )
        ...
        if opts.output_txt:
            ...
            opts.output_txt.write_text(
                "\n".join([p["paper_id"] for p in missing_papers]), encoding="utf-8"
            )
        ...
        if opts.output_paths:
            ...
            static_md_root = (
                (opts.static_export_dir / "md").resolve() if opts.static_export_dir else None
            )
            paths: list[str] = []
            unresolved = 0
