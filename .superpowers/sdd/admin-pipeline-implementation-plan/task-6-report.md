...
Follow-up commits: `fix(frontend): harden admin pipeline lifecycle`; re-review fix; final-gate fix is this commit
...
- Final-gate hardening: restore validation is single-flight; upload initialization watches authoritative auth/config state, deduplicates batch loads, preserves user model choices across config refresh, and reinitializes cleanly across logout/relogin; latest auth operation owns `authLoading`.
- Routes and conditional desktop/mobile Admin navigation; public routes remain unchanged.
- Black-box tests for API auth/upload/artifacts, token lifecycle/defaults/aggregate/empty-selection validation, restore single-flight and auth response reordering, direct-route initialization, login/logout/relogin, preview cleanup, polling termination/offline, partial publish, BibTeX correction/no-Bib, stale publish/indexing retry with preview reload, retry/reject, and notification permission.

Verification:

- Focused admin/frontend tests: 26 passed across API, auth store, upload, batch/job workflow, and protected preview suites.
...
- Final-gate suite covers app/view restore coalescing, direct-route defaults/batch load, login/logout/relogin effects, config-refresh selection preservation, and auth-loading ownership.
